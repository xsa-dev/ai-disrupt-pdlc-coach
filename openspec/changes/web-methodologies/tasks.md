...
- [ ] 1.1 Verify the source map for all 12 entries against the whitepaper text/PDF and record exact section and printed-page references (AC: every entry has verified provenance and no unsupported normative claim; dependency: none)
- [ ] 1.2 Define the final registry data, taxonomy, lifecycle stages, applicability limits, and exact related-antipattern names (AC: 12 unique IDs; all mandatory fields populated; absent source limits use «В источнике не указано»; every antipattern name exists in the current registry; dependency: 1.1)

## 2. Regression Tests First

- [ ] 2.1 Add a source/data-integrity test for entry count, IDs, kinds, mandatory fields, source sections/pages, applicability limits, and antipattern references (AC: test fails before the page/registry exists and covers every data contract; dependency: 1.2)
...
- [ ] 3.2 Implement the single JavaScript registry with the fixed 12 source-grounded entries (AC: source/data-integrity test passes; dependency: 1.2, 3.1)
...
- [ ] 4.3 Render related antipattern names only from the validated registry mapping (AC: no broken or invented antipattern reference is present; dependency: 1.2, 3.3)
